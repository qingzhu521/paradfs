...
pub fn dfs(
    ...
    temp_ans: &mut Option<&mut Vec<GraphPath>>,
    path: &mut GraphPath, 
    rev: bool, 
    ...
    if now == target {
        ...
        // 停止条件3
        if let Some(tans) = temp_ans {
            tans.push(path.clone())
        }
        return;
    }
    visit.insert(now);
    ...
    if let Some(nodes) = next {
        for v in nodes.iter() {
            if !visit.contains(v) {
                path.push(*v);
                dfs(*v, target, k - 1, graph, result, temp_ans, path, rev, visit);
                path.pop();
            }
        }
    }
